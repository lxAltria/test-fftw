...
static void do_twiddle(triggen *t, INT ir, INT m, INT vn, R *xr, R *xi)
{
     ...
     INT im, iv;
    // if(myRank == 0) printf("ir %d\n", ir);

    // if(ir != 0) printf("pe=%d ir=%d m=%dn", myRank, ir, m);
...
static void apply_ddft_first(const plan *ego_, R *I, R *O)
{
     ...
     // if(myRank == 0) printf("dft-rank1 starts\n");
     // fflush(stdout);
     //double elapsedTime;
     ...
     const ctP * myCld = (const ctP *) ego->cld_dft;
     fft2Radix = myRadix / myCld->r;
     if(fft2Radix != myCld->r)
     {
        dmr = true;
     }
     ...
     for (im = ego->xmin; im <= mmax; ++im) {
      // do twiddle while communication
        // if(myRank == 0) printf("im %d\n", im);
